...
- ✅ **`mcp-server`** — deploy and configure the AAP 2.7 MCP server so AI
  coding assistants (Claude Code, Cursor) can query and control AAP via the
  Model Context Protocol. Includes the 2.6→2.7 upgrade path, direct CR
  creation fallback, token setup, and Claude Code `.mcp.json` wiring.
  Companion doc: `docs/mcp-server-guide.md` (value prop + getting started).
...
- ✅ **Workflow node** — `install-dynatrace-windows` / `install-dynatrace-linux`
  fire in parallel with the configure chain immediately after Provision VM
  (not gated behind Patching). Both feed into Update RITM (success) as
  `always` parents.
- ✅ **Job template** — two JTs (Windows + Linux, separate Machine credentials).
  Both also carry `DC1.Azure - ServiceNow` for the `snow_log` role.
- ✅ **Real-time audit proof via `snow_log`** — each DT playbook posts 3 work
  notes per host to the RITM as tasks complete: install check, install result,
  and audit proof (version, tenant, host group, connected status). Replaces
  the previous `set_stats` → deferred `update_ritm.yml` path. See
  [`docs/snow-log.md`](docs/snow-log.md).
...
| 2026-06-09 | **AAP MCP server deployed + `/mcp-server` skill + value-prop doc** (Phase 15) — AAP upgraded from 2.6→2.7 (operator v2.7.0, Controller 4.8.0, EDA 1.2.8, Hub 4.12.1) to unlock the MCP server. Deployed via direct `AnsibleMCPServer` CR creation (bypassing a gateway operator reconciliation bug on "Remove stale routes"). Read-only mode (`allow_write_operations: false`). New repo-based skill `.claude/skills/mcp-server/SKILL.md` covers the full deploy-to-configure flow including the 2.6→2.7 upgrade path, EDA Redis ACL mismatch fix, and direct-CR fallback. Companion doc `docs/mcp-server-guide.md` frames the business value for customers and SE productivity use cases. MCP env vars added to `docs/dev-environment.sh.example`; `.mcp.json` gitignored (contains bearer token). | MCP makes AAP queryable and controllable by AI agents (Claude Code, Cursor) — Red Hat's "trusted execution layer for agentic AI" story. Dual value: customer demo asset (AI + governance) + SE productivity tool (check jobs, query inventories without leaving the terminal). Upgrade gotchas documented in the skill: operator 2.6 has the CRD but can't deploy MCP; EDA Redis upgrades without creating the `eda` ACL user (fix: set username to `default`); stale 2.6 routes block reconciliation. |
...
| 2026-06-06 | **`snow_log` role: real-time, per-host ServiceNow ticket logging** (AB#148) — reusable role (`playbooks/roles/snow_log/`) that any playbook can call to post a work note to any ServiceNow ticket type (RITM, incident, change request). Guarded on `ticket_number` (no-ops on non-SNow triggers), non-breaking (rescue on failure), delegates to localhost. First consumers: Dynatrace OneAgent playbooks post 3 notes per host (install check, install result, audit proof). Replaces the `set_stats` → `update_ritm.yml` path which was unreliable on workflow convergence retries. DT nodes moved parallel with the configure chain (off Provision VM, not after Patching). Tenant-side entity verification dropped (DT host registration takes >6 min; audit proof is sufficient). 10 PRs (#108–117) |
