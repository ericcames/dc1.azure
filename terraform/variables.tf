...
# ---------------------------------------------------------------------------
# OS selection — which VM(s) to provision. Drives count-based conditionals on
# the Windows and Linux resource blocks.
# ---------------------------------------------------------------------------

variable "os_type" {
  description = "Which OS to provision: windows only, linux (RHEL 9) only, or both."
  type        = string
  default     = "windows"
...
# ---------------------------------------------------------------------------
# Linux admin credentials — SSH key is generated by Terraform (tls_private_key).
# ---------------------------------------------------------------------------

variable "linux_admin_username" {
  description = "Admin username on the Linux VM. Azure convention is azureuser."
  type        = string
  default     = "azureuser"
}

variable "linux_ssh_public_key" {
  description = "SSH public key content (e.g. from ~/.ssh/id_rsa.pub) injected into the Linux VM for key-based auth. The default is a throwaway key that satisfies Terraform validation on destroy (never used for a real VM)."
  type        = string
  default     = "ssh-rsa  placeholder@teardown"
}
...
variable "linux_image" {
  description = "Marketplace image reference for the Linux VM. Default = RHEL 9 LVM Gen2 (PAYG)."
  type = object({
    publisher = string
    offer     = string
    sku       = string
